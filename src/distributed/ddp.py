...
class DataParallelDistributedBackend(DistributedBackend):
    ...
    def get_adjusted_args_for_process(self, args):
        ...
        print(f'\t[ddp.py] rank = {self.local_rank}: batch-size = {args.batch_size}, '
              f'acc-steps (pre) = {acc_steps_copy}, '
              f'acc-steps-div = {acc_steps_div}, '
              f'acc-steps (post) = {args.acc_steps}')
        return args
