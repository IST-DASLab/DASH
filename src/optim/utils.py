...
from ista_daslab_optimizers import DashLayerwise
...
class ElapsedTime:
    ...
    opt_step: float = 0.0
    opt_total: float = 0.0
    bw_fw_opt_step: float = 0.0
    bw_fw_opt_total: float = 0.0

    def update(self, elapsed_fw, elapsed_bw, elapsed_opt):
        self.fw_step = elapsed_fw
        self.fw_total += elapsed_fw

        self.bw_step = elapsed_bw
        self.bw_total += elapsed_bw

        self.opt_step = elapsed_opt
        self.opt_total += elapsed_opt

        self.bw_fw_opt_step = elapsed_fw + elapsed_bw + elapsed_opt
        self.bw_fw_opt_total += (elapsed_fw + elapsed_bw + elapsed_opt)

    def log_wandb(self, iter, stdout=False):
        if stdout:
            items = ','.join([
                f'it={iter}',
                f'bw={self.bw_step:.4f}/{self.bw_total:.4f}',
                f'fw={self.fw_step:.4f}/{self.fw_total:.4f}',
                f'opt={self.opt_step:.4f}/{self.opt_total:.4f}',
                f'bw_fw_opt={self.bw_fw_opt_step:.4f}/{self.bw_fw_opt_total:.4f}',
            ])
            # print(f'[{dist.get_rank()}]{items}')
        wandb.log({
            'elapsed/iter': iter,

            'elapsed/bw_step': self.bw_step,
            'elapsed/bw_total': self.bw_total,

            'elapsed/fw_step': self.fw_step,
            'elapsed/fw_total': self.fw_total,

            'elapsed/opt_step': self.opt_step,
            'elapsed/opt_total': self.opt_total,

            'elapsed/bw_fw_opt_step': self.bw_fw_opt_step,
            'elapsed/bw_fw_opt_total': self.bw_fw_opt_total,
        })
...
# def save_checkpoint_original(model, opt, scheduler, itr, ckpt_dir: Path):
...
#         "itr": itr,
#     }
#     if not isinstance(opt, DistributedShampoo):
#         checkpoint["optimizer"] = opt.state_dict()
#     ckpt_dir.mkdir(exist_ok=True, parents=True)
#     torch.save(checkpoint, ckpt_dir / "main.pt")

# def load_checkpoint_original(model, opt, scheduler, ckpt_path, device):
#     if isinstance(model, torch.nn.parallel.DistributedDataParallel):
#         model = model.module
#
...
def save_checkpoint(model, opt, scheduler, itr, ckpt_dir: Path, is_master):
    if isinstance(model, torch.nn.parallel.DistributedDataParallel):
        model = model.module
    ...
    if is_master:
        torch.save(checkpoint, ckpt_dir / "main.pt")
    if not isinstance(opt, (DistributedShampoo, DashLayerwise)):
        torch.save(opt.state_dict(), ckpt_dir / f"optimizer-rank={dist.get_rank()}.pt")
    open(ckpt_dir / f'saved@iter={itr:06d}.txt', 'w').close()

def load_checkpoint(model, opt, scheduler, ckpt_path: Path, device):
    if isinstance(model, torch.nn.parallel.DistributedDataParallel):
        model = model.module

    ckpt_main = torch.load(ckpt_path / "main.pt", map_location=device, weights_only=False)
    model.load_state_dict(ckpt_main["model"])
    scheduler.load_state_dict(ckpt_main["scheduler"])
    itr = ckpt_main["itr"]

    ckpt_opt = torch.load(ckpt_path / f"optimizer-rank={dist.get_rank()}.pt", map_location=device, weights_only=False)
    opt.load_state_dict(ckpt_opt)

    return itr

def save_wandb_state(ckpt_dir: Path, is_master):
    if is_master:
        wandb_state = dict(
            project=wandb.run.project,
            group=wandb.run.group,
            job_type=wandb.run.job_type,
            id=wandb.run.id,
            name=wandb.run.name,
            entity=wandb.run.entity)
        torch.save(wandb_state, ckpt_dir / "wandb.pt")

def load_wandb_state(ckpt_dir: Path):
    return torch.load(ckpt_dir / "wandb.pt", weights_only=False)
